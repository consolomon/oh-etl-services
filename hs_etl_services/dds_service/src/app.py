...
if __name__ == '__main__':
    ...
    scheduler.add_job(func=chat_proc.run, trigger="cron", hour="0-23")
    scheduler.add_job(func=position_proc.run, trigger="cron", hour="0-23")
    scheduler.add_job(func=tech_proc.run, trigger="cron", hour="0-23")
    scheduler.add_job(func=resume_vacancy_proc.run, trigger="cron", minute="5")
    scheduler.add_job(func=vacancy_position_tech_proc.run, trigger="cron", minute="20-50/10")
    scheduler.add_job(func=resume_position_tech_proc.run, trigger="cron", minute="20-50/10")
    scheduler.start()
